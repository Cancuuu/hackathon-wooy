...
import { useAllPoolsKeyedByChainId } from '@wooy/hooks'

import TokenFaucetAbi from '@wooy/pool-contracts/abis/TokenFaucet'
...
import {
  ...
  THIRD_POLYGON_USDT_FAUCET_ADDRESS,
  FIRST_POLYGON_USDC_FAUCET_ADDRESS
...
export const useAllTokenFaucetsByChainId = () => {
  ...
  const data = useMemo(() => {
    ...
    allChainIds.forEach((chainId) => {
      ...
      const poolTokenFaucets = poolTokenFaucetsByChainId[chainId]?.map((tokenFaucet) => ({
        ...tokenFaucet,
        isPod: false
      }))
      if (poolTokenFaucets) {
      ...
      const podTokenFaucets = podTokenFaucetsByChainId[chainId]?.map((tokenFaucet) => ({
        ...tokenFaucet,
        isPod: true
      }))
      if (podTokenFaucets) {
