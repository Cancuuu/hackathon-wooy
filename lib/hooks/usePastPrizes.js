...
import { useCurrentPool, useReadProvider } from '@wooy/hooks'
...
export const usePastPrizes = (pool, page, pageSize = PRIZE_PAGE_SIZE) => {
  ...
  const { readProvider, isReadProviderReady: readProviderReady } = useReadProvider(chainId)
...
export const usePastPrize = (pool, prizeNumber) => {
  const chainId = pool?.chainId
  const { readProvider, isReadProviderReady: providerIsLoaded } = useReadProvider(chainId)
