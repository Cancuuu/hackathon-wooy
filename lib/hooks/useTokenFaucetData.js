...
import { useReadProvider } from '@wooy/hooks'
...
export const useTokenFaucetData = (tokenFaucetAddress) => {
  ...
  const { readProvider, isReadProviderReady: readProviderIsLoaded } = useReadProvider(chainId)
  ...
  const enabled = tokenFaucetAddress && !addressError && !isZeroAddress && readProviderIsLoaded
