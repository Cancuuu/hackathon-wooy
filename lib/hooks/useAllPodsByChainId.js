...
import { podContractAddresses as allPodContractAddressesByChainId } from '@wooy/current-pool-data'
import { batch, contract } from '@pooltogether/etherplex'
import { usePodChainIds, useReadProviders } from '@wooy/hooks'
import { addBigNumbers, calculateUsersOdds, deserializeBigNumbers, sToMs } from '@wooy/utilities'
...
export const useAllPodsByChainId = () => {
  ...
  const { isReadProvidersReady, readProviders: providersByChainId } = useReadProviders(chainIds)

  const enabled = isReadProvidersReady

  return useQuery(
    [QUERY_KEYS.getPods, chainIds],
