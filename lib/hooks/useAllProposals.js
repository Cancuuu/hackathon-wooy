...
function useFetchProposals() {
  ...
  const { readProvider } = useReadProvider(mainnetChainId)
  // const { readProvider, isLoaded: readProviderIsLoaded } = useReadProvider()
  const block = useBlockOnProviderLoad()

  return useQuery(
    [QUERY_KEYS.proposalsQuery, mainnetChainId, block?.blockNumber],
    async () => {
      return getProposals(readProvider, mainnetChainId, block)
    },
    {
